...
function FolderItem({
  ...
  return (
    <li
      ...
      onDragEnter={(e) => {
        if (
          e.dataTransfer.types.includes("application/x-rokki-file") ||
          e.dataTransfer.types.includes(DRAG_MIME_FILE)
        ) {
          e.preventDefault();
          onDragEnter();
        }
  ...
  return (
    <li
      ...
      onDragStart={(e) => {
        ...
        // Set BOTH the legacy mime (folder-move within FilesPane already
        // listens for it) and the canonical cross-pane mime so a Tasks row
        // can sniff and accept the drop.
        e.dataTransfer.setData("application/x-rokki-file", file.id);
        setDragPayload(e.dataTransfer, "file", file.id, file.filename);
        onDragStart();
      }}
      onDragEnd={onDragEnd}
      className={cn(
